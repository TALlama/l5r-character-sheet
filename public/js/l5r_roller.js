...
class DiceTray {
  ...
  addDice(rollbox, {roll} = {roll: false}) {
    this.dicePool.dice.forEach(die => {
      ...
      dieUi.classList.add(`die---${die.dieType}`);
      ...
      rerollLink.classList.add('die--action');
      rerollLink.classList.add('die--action---reroll');
...
document.addEventListener('xclick', event => {
